      ...
      <main>
        ...
        <section id="about" className="section-rule bg-background py-24 sm:py-32">
          <div className="mx-auto grid max-w-7xl gap-14 px-5 sm:px-8 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
            ...
            <div><p className="max-w-3xl text-lg leading-8 text-muted-foreground">Computer Science (AI &amp; DS) undergraduate with hands-on experience in Python, SQL, Power BI, and Advanced Excel through a data analytics internship and multiple analytics projects. Skilled in data cleaning, exploratory data analysis, dashboard development, and machine learning fundamentals, with practical exposure to complex SQL queries and interactive Power BI/Excel dashboards. Certified in data analytics and visualization through Deloitte, Accenture, and SmartBridge.</p><div className="mt-10 flex flex-wrap gap-2">{["Python", "SQL", "Power BI", "Advanced Excel", "Data Cleaning", "EDA", "Dashboard Development", "Machine Learning Fundamentals"].map((item) => <span key={item} className="rounded-full border border-border bg-card/50 px-3.5 py-2 text-sm text-foreground transition-colors hover:border-primary/45 hover:text-primary">{item}</span>)}</div></div>
          </div>
        </section>
...
function AnalyticsMindset() {
  return <section className="section-rule relative overflow-hidden bg-background py-24 sm:py-32"><div className="absolute inset-0 data-grid opacity-20" /><div className="relative mx-auto max-w-7xl px-5 sm:px-8"><div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-center"><div><p className="font-mono text-xs uppercase tracking-[0.2em] text-secondary">ANALYTICS MINDSET</p><h2 className="mt-4 text-4xl font-extrabold tracking-tight text-foreground sm:text-5xl">Make the signal easier to see.</h2><p className="mt-5 max-w-lg leading-7 text-muted-foreground">A visual language for the work: extract, clean, explore, model, and communicate.</p></div><div className="glass rounded-3xl p-5 sm:p-8"><div className="flex flex-wrap items-center justify-center gap-3 sm:gap-5">{[[Database, "Extract", "primary"], [ScanLine, "Clean", "cyan"], [BarChart3, "Explore", "green"], [BrainCircuit, "Model", "violet"], [MessageCircle, "Communicate", "amber"]].map(([Icon, label, color], index) => <div key={label} className="flex items-center gap-3"><div className={`grid size-12 place-items-center rounded-2xl border border-${color}/30 bg-${color}/10 text-${color}`}><Icon size={19} /></div><span className="text-xs font-bold text-foreground">{label}</span>{index < 4 && <ChevronRight size={15} className="hidden text-muted-foreground sm:block" />}</div>)}</div><div className="mt-8 flex h-24 items-end gap-2 rounded-2xl border border-border bg-background/50 px-5 pb-4 pt-5">{[28, 43, 37, 62, 54, 76, 68, 91, 82, 96].map((height, index) => <span key={index} className={`animate-pulse-line w-full rounded-t-sm ${index > 6 ? "bg-secondary/70" : "bg-primary/55"}`} style={{ height: `${height}%`, animationDelay: `${index * 70}ms` }} />)}</div></div></div></div></section>;
}